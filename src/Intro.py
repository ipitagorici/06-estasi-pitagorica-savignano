...
class Intro(Scene):
    def construct(self):
        cap = cv2.VideoCapture("./assets/SLOWStelleOrbitanti.mp4")
        flag = True
        frame_imgs = []
        ...
        logo = ImageMobject("./assets/logo_BP_CLEAN.png").scale(.25).next_to(title, UP, .3).set_color(WHITE)
        text = Group(title, subtitle, title_shadow, subtitle_shadow, logo).center()

        background = ImageMobject("./assets/background.png")
        background.scale_to_fit_width(self.camera.frame_width).set_z_index(-3).set_opacity(.5)
        castle = ImageMobject("./assets/castello-longiano.png").set_z_index(0).scale_to_fit_width(self.camera.frame_width).to_edge(DOWN, -1)
        pythagoras = ImageMobject("./assets/pythagoras_nobg.png").set_z_index(-1).scale(.85).shift(UP * 1.5 + RIGHT * 3.5)
